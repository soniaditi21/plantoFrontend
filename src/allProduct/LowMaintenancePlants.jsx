  ...
  return (
    <Box>
      <Box className={classes.topContainer}>
        <Typography className={classes.title}><b><h1>Low Maintenance Plants</h1></b></Typography>
        <Typography className={classes.titleDescription}><h3>We brings to you a diverse range of low maintenance plants online for both indoors and outdoors. Buy natural plants at the best price and start gardening enthusiastically.</h3></Typography>
      </Box>
     ...
     >
      {lowmaintenanceplants.map(lowmaintenanceplant => (
         ...
         <Grid item md={4} >
          ...
          <Box > 
          <img className={classes.img} src={lowmaintenanceplant.url1} alt="" />
          <Typography className={classes.itemTitle}>{lowmaintenanceplant.title.shortTitle}</Typography>
          <Typography className={classes.price}><strike style={{color: 'grey', marginRight: '2%'}}>₹{lowmaintenanceplant.price.strikeprice}</strike> ₹{lowmaintenanceplant.price.actualprice}</Typography>
         <Link to='/cart' style={{textDecoration: 'none'}}>  <Button className={classes.addToCart} onClick={() => lmAddToCartItems(lowmaintenanceplant.id)} style={{ 
            backgroundColor: '#096C6E',
        color: 'ivory',
